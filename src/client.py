...
with inputColumn:
    ...
    if st.button('Send'):
        ...
        #Add connection logic
with outputColumn:
    placeholder_position_data = pd.DataFrame(0,index=np.arange(20),columns=['Position'])
    placeholder_velocity_data = pd.DataFrame(0,index=np.arange(20),columns=['Velocity'])
